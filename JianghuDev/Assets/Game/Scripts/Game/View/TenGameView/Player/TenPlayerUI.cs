...
public class TenPlayerUI : BaseViewWidget
{
    ...
    public GameObject mRetObj;     //结果牌型go
    public UILabel mCardTypeLabel;//牌型
    ...
    protected TenBasePlayerInfo mBasePlayerInfo;//基础信息节点
    ...
    private TenPlayerHandCardsType mCardsType;//牌型
    ...
    {
        if (SeatId>0 && TenModel.Inst.mGameedSeatIdList.Contains(SeatId)) {
            Global.Inst.GetController<TenGameController>().SendGetPlayerInfo(TenModel.Inst.mPlayerInfoDic[mSeatId].userId, mSeatId);
        }        
    }
    ...
    public void InitPlayer(string head, string name, string uid, float score, bool ready = false, bool offline = false)
    {
        if (mBasePlayerInfo == null)
        {
            //mBasePlayerInfo = Assets.InstantiateChild(mHeadParent, Global.Inst.GetController<TenGameController>().mView.mPlayerBaseItem.gameObject).GetComponent<TenBasePlayerInfo>();
            Debug.Log(mBasePlayerInfo);
        }
    ...
    /// </summary>
    /// <returns></returns>
    public TenPlayerHandCardsType GetCardsType() {
        return mCardsType;
    }
    ...
    protected void PlayTxtVoiceChat(SendReceiveGameChat chat)
    {
        ...
        for (int i = 0; i < mRulelist.Count; i++)
        {
            ...
            if (config.id == chat.faceIndex)
            {
                ...
                int sex = 1;
                TenPlayerInfo player = null;

                if (TenModel.Inst.mPlayerInfoDic.TryGetValue(chat.fromSeatId,out player)) {
                    sex = player.sex;
                }
    ...
    public string GetShowPrompt(int point)
    {
        string retStr = "";

...
public class ChatScrollItem : MonoBehaviour
{
    public int indexInScroll;//当前第几个,在scroll里面的顺序
    public int indexInData;//数据里面的位置
    public float contentHeight;//占用的渲染高度


}

public class ChatScrollView : GameEventUnity
{
    ...
    private void InitData()
    {
        ...
        mIsLoadOver = false;
        mTran.localPosition = mInitPanelPos;
    ...
    IEnumerator InitItems()
    {
        ...
        int start = mAllCount <= ShowCount ? 0 : (mAllCount - ShowCount);
        int count = start + ShowCount;
        ...
        if (count > mAllCount)
        {
            ...
            mMaxIndex = count == 0 ? 0 : count - 1;//滑动底下的index
        //初始化panel的位置和裁剪
    ...
    private void Reset()
    {
        ...
        for (int i = 0; i < mItemList.Count; i++)
        {
            ClearAllChild(mItemList[i].transform);
            mItemList[i].contentHeight = 0;
        }
        mItemIndexDic.Clear();
        transform.localPosition = mInitPanelPos;
        mPanel.clipOffset = mInitPanelClip;
        mNextUpPos = 0;//下一次比较的位置
        mNextDownPos = 0;//下一次比较的位置
        SQDebug.Log("初始位置" + mInitPanelPos);
    }
